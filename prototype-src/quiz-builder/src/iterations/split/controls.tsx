...
export function FineTuning({ state, onChange }: { state: SplitState; onChange: (patch: Partial<SplitState>) => void }) {
  return (
    <div style={wrap}>
      <div>
        <div style={label}>Preview stage</div>
        <Seg
          value={state.previewStage}
          onPick={previewStage => onChange({ previewStage })}
          options={[
            { id: 'quiz', label: 'Quiz' },
